...
* Cluster resource with * verb on Rancher Cluster Role
  + Mapped to NeuVector fedAdmin role on NeuVector fed-master cluster  (This means users cannot map a Rancher Cluster role to a NeuVector admin role if NeuVector is on a master cluster)
