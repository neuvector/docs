...
7. Enter the same SAML Redirect URI into the “Relying party trust identifier” field and click “Add”; then select “Next” to continue. This identifier is the audience URI (SP entity ID) that AD FS puts in the audience restriction of the assertion.

![adfsSetup](adfs7.png)

:::note
NeuVector uses its SAML Redirect URI as both the assertion consumer service URL and the audience URI (SP entity ID); it is not configurable in the NeuVector console. Enter that same value in both the “Relying party SAML 2.0 SSO service URL” and the “Relying party trust identifier” fields so the audience of the assertion matches what NeuVector expects.
:::
...
Log in to the NeuVector console as an administrator and go to **Settings > SAML Settings**, then fill in the following fields.
...
Click **Add to Top** and enter the group name of the users together with the Global Role and Namespace Roles to assign to its members.

Every user needs a role, assigned either by the default role or by the role map. A user who ends up without a role cannot log in, even with the correct credentials.
...
| Setting | Value |
| ------- | ----- |
| Identity Provider Single Sign-On URL | Requires HTTPS protocol |
