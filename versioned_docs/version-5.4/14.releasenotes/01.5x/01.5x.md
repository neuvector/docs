...
##### Security Advisories
* [NeuVector is shipping cryptographic material into its binary](xxxx)
* [Telemetry sender is vulnerable to MITM and DoS](xxxx)
* [Sensitive information exposure in NeuVector manager container logs](xxxx)
...
+ Anonymous telemetry data (number of nodes, groups, rules) is now reported to a Rancher cloud service upon deployment to assist the project team in understanding usage behavior. This can be disabled (opt-out) in UI or with configMap (No_Telemetry_Report) or REST API.
  +  You can disable the Report anonymous cluster data option, which is enabled by default. To change this setting, go to **Settings** → **Configuration** → **Report anonymous cluster data** in the NeuVector UI. For more information, refer to [Telemetry sender is vulnerable to MITM and DoS](../../../../docs/16.security_advisories/01.security_advisories/cve.md#telemetry-sender-is-vulnerable-to-mitm-and-dos).
