...
NeuVector offers two process and file protection modes: Zero-Drift enforces strict adherence to the original container image by only allowing files and processes originating from the parent process present in the original container image and observed during the learning phase; Basic provides broader protection learning and allowing all running processes inside the container.
...
Starting on version 5.4.x, Zero-drift does require processes to be learned or manually added to an allow-list. Now Zero-drift will only allow what has been learned and allowed in the process profile rules.
