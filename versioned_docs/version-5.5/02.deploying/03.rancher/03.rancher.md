...
The NeuVector core Helm chart must already be deployed on the target cluster before the UI Extension will display anything for it. If NeuVector is not yet installed, complete the [Deploy NeuVector](#deploy-neuvector) steps below first, then return here to install the extension.
:::
