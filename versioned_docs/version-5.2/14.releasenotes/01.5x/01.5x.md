...
+ * The federation of scan results to remote/managed clusters is now configurable. Please note that the default setting is `OFF`. In previous versions, this behavior was enabled by default. For more information, refer to [Federating Results from CI/CD Scanners (Optional)](../../04.navigation/02.multicluster/02.multicluster.md#_federating_results_from_ci_cd_scanners_optional).
