...
#### Overriding a Workload's Service

By default, NeuVector derives the service (and the corresponding learned group, prefixed with `nv.`) for each workload from orchestration metadata such as the deployment/pod name. A workload can override this derivation by setting the label `io.neuvector.service.name` on the container or pod. For example, a pod labeled `io.neuvector.service.name=myapp` in namespace `prod` is placed into the group `nv.myapp.prod` regardless of its pod name.

On Kubernetes the label value is lowercased and normalized to DNS-1123 form before use, so `io.neuvector.service.name=MyApp` results in the group `nv.myapp.prod`.

:::warning
Honoring this label is **disabled by default**. NeuVector ignores `io.neuvector.service.name` unless the feature is explicitly enabled.
:::

To enable it, set the environment variable `NV_ENABLE_CUSTOM_SVC_NAME` to `"1"` (or `"true"`) on **both** the Controller and the Enforcer. Both components evaluate the label, so setting it on only one leads to inconsistent group assignment. With the Helm chart, add the variable to the `controller.env` and `enforcer.env` passthrough lists in your values file:

```yaml
controller:
  env:
    - name: NV_ENABLE_CUSTOM_SVC_NAME
      value: "true"
enforcer:
  env:
    - name: NV_ENABLE_CUSTOM_SVC_NAME
