...
User can configure following DDoS detection thresholds:
...
:::note
We sample the past 60 seconds traffic in average to see whether the metric break the threshold or not for the DDoS detection feature, measurement in protect mode is most meaningful because enforcer becomes inline at the data-path. Learned, User-created, CRD created and federal created container group can configure DDos detection thresholds.
:::
