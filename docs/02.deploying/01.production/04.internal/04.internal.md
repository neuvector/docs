...
<TabItem value="regenarate_cert_from_builtin" label="Regenarate certificate when built-in certificate is being used">
If you didn't replace internal certificate before and want to migrate to a new set of certificate, follow the steps below:

+ Check if you already have internal certificate generated automatically.
...
If you can see the secret, but cannot find these secrets, consider enabling `internal.autoRotateCert` in helm charts override. This option will generate and rotate your internal certificate automatically.

If you don't use automatically generated internal certificate and can't do so, follow the steps below:

+ Follow the steps in `New certificate` tab to use kubernetes secret to manage internal certificate.  Instead of generating a new certificate, use the certificate, `old-ca.crt`, `old-tls.crt` and `old-tls.key`, retrieved below:
...
+ Make sure that all components are running without a problem.

+ After that, follow the steps in `Regenarate certificate files and add SANs` tab and migrate to your own certificate.
