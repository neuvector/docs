...
> (Optional) Set the value to "1" (or "true") to allow a workload to specify its NeuVector service/group via the `io.neuvector.service.name` label. Because the label is evaluated by both the controller and the enforcer, this variable must be set on **both** components. See [Overriding a Workload's Service](/policy/groups#overriding-a-workloads-service) for details.
