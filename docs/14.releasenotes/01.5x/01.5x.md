...
:::note
If it's upgrade from NeuVector pre-5.6.0 to 5.6.2, all NeuVector scanner pods needs to be restarted for supporting Critical CVE severity.
:::
...
* [Admission Control Bypass via Hardcoded Sidecar Image Exemption](https://github.com/neuvector/neuvector/security/advisories/GHSA-78r4-3wfq-r2xm)
* [Logout bypass via alternate JWT spelling](https://github.com/neuvector/neuvector/security/advisories/GHSA-wcx5-mq6c-c54j)
* [The SAML and OpenID SSO login does not have proper identifier for login users](https://github.com/neuvector/neuvector/security/advisories/GHSA-c6rx-pmvf-m3jx)
* [SAML Audience Confusion Allows Cross-SP Authentication ](https://github.com/neuvector/neuvector/security/advisories/GHSA-wgg5-24xq-px35)
* [OS Command Injection in Packet-Capture (Sniffer) Filter leading to Remote Code Execution on Kubernetes Nodes ](https://github.com/neuvector/neuvector/security/advisories/GHSA-vr77-8vmq-qfmj)
...
- **feat**: Move the readiness probe from `exec` to `httpGet`.
- **NVSHAS-10024**: Add an option to filter vulnerabilities based on the base OS image.
...
- [#2227](https://github.com/neuvector/neuvector/issues/2227): UBI controller images do not work in newer OpenShift environments.
...
:::note
In NeuVector v5.4.6 and later, the Compliance feature requires NeuVector Prime and must have controller.`prime.enabled: true` set in the values.yaml file. If the Compliance feature is missing after upgrade, refer to[Compliance Feature Behavior Across NeuVector Versions](../../12.troubleshooting/01.troubleshooting/01.troubleshooting.md#compliance-feature-behavior-across-neuvector-versions).
:::

##### New Features

* **NVSHAS-10085**:	Add a configuration setting on UI for allowing groups switch between least privileged & more privileged.
* **NVSHAS-10084**:	Move admission control related crd structs in `nvsecurityrule_def.go` from `resource/` to `api/` for dependency issue when building `neuvector-kubewarden-policy-converter`.
* **NVSHAS-10073**:	Add a new field for exported admission control rules in the yaml file.
...
* [Removal of hard-coded encryption key](https://github.com/neuvector/neuvector/security/advisories/GHSA-h773-7gf7-9m2x)
* [Enforce TLS verification for requests to Telemetry server](https://github.com/neuvector/neuvector/security/advisories/GHSA-qqj3-g7mx-5p4w)
* [Check consul ports for enforcer](https://github.com/neuvector/neuvector/security/advisories/GHSA-c8g6-qrwh-m3vp)

Refer to the [Security Advisory and CVEs](docs/cve.md) documentation for more information.
...
## References of fixed CVEs:

The fixed CVEs are:

* [Telemetry sender is vulnerable to MITM and DoS](https://github.com/neuvector/neuvector/security/advisories/GHSA-qqj3-g7mx-5p4w)
* [Enforcer is vulnerable to Command Injection and Buffer overflow](https://github.com/neuvector/neuvector/security/advisories/GHSA-c8g6-qrwh-m3vp)

For more information, refer to [Security Advisories and CVEs](docs/cve.md)
...
+ Anonymous telemetry data (number of nodes, groups, rules) is now reported to a Rancher cloud service upon deployment to assist the project team in understanding usage behavior. This can be disabled (opt-out) in UI or with configMap (No_Telemetry_Report) or REST API.
  +  You can disable the Report anonymous cluster data option, which is enabled by default. To change this setting, go to **Settings** → **Configuration** → **Report anonymous cluster data** in the NeuVector UI.  
