...
For authentication to be successful, the time between the K8s Nodes and the ADFS server would need to be the same.This is very likely due to time sync or clock drift issues. 

The recommendation is to use an NTP server, with equal time settings between all the servers involved. Please check and confirm that both ends are synchronized and that there are no delays of more than 10 seconds. You can use linux and windows commands to check dates, times and NTP server activity.

To confirm this assumption, you can try the following WorkAround:
