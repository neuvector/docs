...
From v5.4.0 NeuVector provide "View all scanned images". Users don't have to look into registry to see a specific image. Instead, users can click the last row to get all scanned image list in one stop.

![Registry](view-all-scanned-images.png)

Users can also land to Registries page and open specific image from the hyperlink from Containers page. If the image name is not clickable, it means that the image has not been scanned yet.
