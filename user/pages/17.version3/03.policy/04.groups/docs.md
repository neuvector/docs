...
This menu is the key area to view and manage security rules and customize Groups for use in rules. It is also used to switch modes of Groups between Discover, Monitor, and Protect. Please see the following individual sections for explanations of Customer Compliance Checks, Network Rules, Process and File Access Rules and DLP detection.
 
NeuVector automatically creates Groups from your running applications. You can also manually add them while in any mode, Discover, Monitor, or Protect. Network and Response Rules require these Group definitions. For automatically created Groups ('learned' groups), NeuVector will learn the network and process rules and add them while in Discover mode. Custom Groups will not auto-learn and populate rules.
...
Groups can be manually added by entering the criteria for the group. Groups can be created by:
+ **Images**
...
A group can be created with mixed criteria types, except the 'address' type, which cannot be used together with other criteria. Mixed criteria enforces an ‘AND’ operation between the criteria, for example label service_type=data AND image=mysql. Multiple entries for one or criteria are treated as OR, for example address=google.com OR address=yahoo.com.
