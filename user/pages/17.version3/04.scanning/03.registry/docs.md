---
title: Registry Scanning Details
taxonomy:
    category: docs
---

NeuVector enables scanning & compliance through vulnerability scanning and running of the CIS benchmarks for security.

Registry scanning requires that the NeuVector Allinone or Controller container be deployed on a host/node. Please see the Installation/Deployment section for how to deploy the NeuVector container. Configure registry scanning from the NeuVector console after logging in to the manager.

Note: Registry image scanning is performed by the controller and the image is pulled and expanded in memory. If expanded image sizes larger than 500MB are expected, consider increasing the controller memory to 1.5GB or more to provide capacity and headroom for the controller.
...
The Registry Scan feature allows to scan the images on public and private docker registries that are based on Native Docker, Amazon ECR, Redhat/Openshift, jFrog, Microsoft ACR, Sonatype Nexus, Harbor and Google cloud registry.  The scan report for the image comprises of the vulnerability status of various packages and binaries in the image. If selected, the result is shown as a layered image scan which shows the vulnerabilities in each layer. The brief summary of the scan report can be sent via webhook using the Response rule configuration in Policy -> Response Rules, or by Syslog by configuring a syslog server in Settings -> Configuration. Results can also be viewed in the Event logs.
...
Example showing layer scan result of an image, which shows vulnerabilities of each layer, layer size and commands run on each layer.
![layered](layered.png)
...
+ Choose JFrog Artifactory as type
+ Give a unique name to the registry
+ Type the registry URL with port, for example http://docker-local.10.1.7.122:8081/
    - Repository name is pre-appended to the hostname and should be accessible by controller
...
+ Add the repository as a filter in the below format
    ...
    - To scan all tags for a repository, for example alpine, use alpine:&#42;
    - The wildcard must be preceded by the full name, path, or starting string
    - &#42; for scanning all tag
    - Enable Xray if supported
    - Add Xray server address
    - Provide a username and password if required by the Xray server
+ Wildcard support of JFrog registry in subdomain mode
    - In put the artifactory URL as the registry URL
    - &#42; for scanning all subdomains, or specify subdomains with wildcard match for images


Adding a single JFrog local repository 
![jfrog](jfrog102.png)

Adding a JFrog virtual registry to scan all tags
![jfrogvirtual](jfrog103.png)
...
+ Give unique name to the registry
+ Type registry URL. Sample https://gcr.io/ 
